...
class TextShaper:
    ...
    def shapeWord(learner, downsampling_factor=None):
        ...
        for letter in learner.current_word:
            ...
            # normalyze x & y
            x_shape = [(x - min(x_shape)) * SIZESCALE_WIDTH * scale_factor for x in x_shape]
            y_shape = [(y - max(y_shape)) * SIZESCALE_HEIGHT * scale_factor for y in y_shape]

            # offset in X
            # if not first letter
            if len(paths) != 0:
                # if first point of letter
                if len(path) == 0:
                    offset_x = maxX # TODO

            # offset in Y
            if len(path) == 0:
                offset_y = -bh/(ah+bh) * SIZESCALE_HEIGHT * scale_factor


            for i, xi in enumerate(x_shape):
                x = xi + offset_x
                y = -y_shape[i] + offset_y
